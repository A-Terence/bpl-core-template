...
export default function FleetCompactTable({ vehicles }: Props) {
  if (!vehicles.length) {
    return (
      <div className="bpl-fleet-table-empty">No vehicles match this filter.</div>
    );
  }
          ...
          return (
            ...
            >
              ...
              <div className="bpl-fleet-cell-transporter">
                <span className="bpl-fleet-dot" style={{ background: isPanic ? 'var(--bpl-red)' : 'var(--tenant-primary, var(--bpl-blue))' }} />
                {v.transporter}
              </div>
